...
| `lang` | no | `en` `fr` `es` `de` `it` | auto | Interface language. Without it: the viewer's browser language, else the language of the country at `lat`/`lng`, else English. The flag in the time bar lets the viewer switch inside the embed too. Added 2026-09-19 (additive, still v1). |
